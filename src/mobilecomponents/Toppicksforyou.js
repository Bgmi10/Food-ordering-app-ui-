     ...
     { data.length ===0 ? <Shimmer /> : <Slider {...settings} className="mr-7 mt-3">
        {carouselData.map((restaurant, index) => (
          <div key={index} className="flex flex-col items-center ml-3 ">
            ...
            <span className=" "> {restaurant.itemDiscount === true ? <div className='absolute top-0 left-[148px] text-[9px] z-10 opacity-20 bg-black text-white p-2 font-extrabold  rounded-lg'>add</div> :null } </span>
